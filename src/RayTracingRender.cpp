...
QSharedPointer<RayTracingRender> RayTracingRender::instance = nullptr;


QSharedPointer<RayTracingRender>& RayTracingRender::GetInstance()
{
	if (instance.isNull())
		instance = QSharedPointer<RayTracingRender>(new RayTracingRender(), RayTracingRender::destory);
	return instance;
}

void RayTracingRender::destory(RayTracingRender* builder)
{
	delete builder;
}

void RayTracingRender::buildBVH() {
...
void RayTracingRender::buildTriangles()
{
...
int RayTracingRender::findSplitAxis(int l, int r, BVHNode& node, float& cost)
{
	int splitIndex = 0;
